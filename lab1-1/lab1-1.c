...
GLuint phongshader = 0, plaintextureshader = 0,
			 lowpassxshader = 0, lowpassyshader = 0,
			 truncshader = 0, combinetexshader = 0;
...
void init(void)
{
	...
	lowpassxshader = loadShaders("plaintextureshader.vert", "lowpassx.frag"); // renders with a lowpass filter horisontally
	lowpassyshader = loadShaders("plaintextureshader.vert", "lowpassy.frag"); // renders with a lowpass filter vertically
	truncshader = loadShaders("plaintextureshader.vert", "trunc.frag"); // renders with a lowpass filter vertically
	combinetexshader = loadShaders("plaintextureshader.vert", "combinetextures.frag"); // renders with a lowpass filter vertically c
...
void display(void)
{
	...
	int i = 0;
	int nrpasses = 100;
	// We want to filter in both directions, therefore we use 2*nrpasses.
	for(; i < 2*nrpasses; i++){
		// Ping pong!
		if(i % 2 == 0)
			runfilter(lowpassxshader, fbo2, 0L, fbo3);
		else
			runfilter(lowpassyshader, fbo3, 0L, fbo2);
	}

	////////////////////////////////////////////////////////////////////////////
	// ADD BLOOM TO SCENE
	runfilter(combinetexshader, fbo3, fbo1, 0L);
